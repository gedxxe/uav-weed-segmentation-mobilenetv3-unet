...
| Proposed loss recipe | Implemented | Proposed-only runners default to `ce_dice` with automatic class weights and `validation_loss=macro_f1` to address class confusion and select checkpoints by the reported class-balanced validation metric. |
| Proposed foreground tuning controls | Implemented | Added consecutive early stopping, `class_weight_strategy=sqrt_inverse`, and `foreground_macro_f1` checkpoint selection for Sorghum+Weed balancing without changing architecture. |
